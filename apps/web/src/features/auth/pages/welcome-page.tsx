import { BriefcaseBusiness, Search } from 'lucide-react';
import { Link } from 'react-router';
import { Brand } from '@/components/shared/brand';
import { MobileShell } from '@/components/shared/mobile-shell';
import { RoleOption } from '@/components/shared/role-option';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { PwaInstallCard } from '@/features/pwa/components/pwa-install-card';

function WelcomePage() {
  return (
    <MobileShell>
      <main className="safe-area-top mx-auto flex w-full max-w-6xl flex-1 flex-col px-5 pb-6 sm:px-6 lg:px-10 lg:pb-10">
        <div className="flex items-center justify-between gap-3">
          <Brand />
          <Badge variant="secondary">Simples de usar</Badge>
        </div>

        <div className="relative mt-10 lg:mt-16">
          <span
            aria-hidden="true"
            className="absolute -top-5 -left-5 h-18 w-2 rounded-r-full bg-accent"
          />
          <p className="mb-2 text-base font-bold text-primary">Seu próximo serviço começa aqui</p>
          <h1 className="max-w-3xl text-[2.6rem] leading-[1.02] font-extrabold tracking-[-0.035em] sm:text-5xl lg:text-6xl">
            Trabalho perto de você.
          </h1>
          <p className="mt-4 max-w-2xl text-lg leading-relaxed text-muted-foreground lg:text-xl">
            Encontre uma oportunidade ou contrate alguém de confiança, sem complicação.
          </p>
        </div>

        <section aria-labelledby="role-heading" className="mt-9 lg:mt-12">
          <h2 className="mb-3 text-base font-extrabold" id="role-heading">
            O que você quer fazer?
          </h2>
          <div className="grid gap-3 md:grid-cols-2">
            <RoleOption
              description="Quero encontrar trabalho"
              icon={Search}
              title="Buscar um serviço"
              to="/cadastro?tipo=trabalhador"
            />
            <RoleOption
              description="Quero encontrar um profissional"
              icon={BriefcaseBusiness}
              title="Contratar alguém"
              to="/cadastro?tipo=contratante"
              tone="accent"
            />
          </div>
        </section>

        <div className="mt-6 max-w-xl">
          <PwaInstallCard />
        </div>

        <div className="mt-auto flex flex-col items-center gap-1 pt-9 text-center lg:items-start lg:text-left">
          <span className="text-base text-muted-foreground">Já tem uma conta?</span>
          <Button asChild variant="link">
            <Link to="/entrar">Entrar no TrampoFácil</Link>
          </Button>
        </div>
      </main>
    </MobileShell>
  );
}

export { WelcomePage };
